...
async function handleApi(req, res, url) {
  ...
  if (req.method === 'GET' && url.pathname === '/api/keywords') {
    sendJson(res, 200, {
      keywords: [...state.keywords].sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0) || a.term.localeCompare(b.term)),
    });
    return;
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/keywords') {
    const body = await readJsonBody(req);
    const keyword = createKeywordRecord(body);
    state.keywords.unshift(keyword);
    pushActivity(`Keyword added: ${keyword.term} (${keyword.language.toUpperCase()} / ${keyword.market.toUpperCase()}).`, 'info');
    await persistState();
    sendJson(res, 201, { ok: true, keyword });
    return;
  }
  ...
  if (projectRoute && req.method === 'DELETE') {
    ...
    state.keywords = state.keywords.filter((item) => item.projectId !== project.id);
  ...
  const keywordRoute = url.pathname.match(/^\/api\/keywords\/([^/]+)$/);
  if (keywordRoute && req.method === 'PATCH') {
    const [, keywordId] = keywordRoute;
    const keyword = state.keywords.find((item) => item.id === keywordId);
    if (!keyword) {
      sendJson(res, 404, { error: 'not_found', message: 'Keyword was not found.' });
      return;
    }

    const body = await readJsonBody(req);
    const updatedKeyword = createKeywordRecord(body, keyword);
    Object.assign(keyword, updatedKeyword, {
      id: keyword.id,
      createdAt: keyword.createdAt,
      updatedAt: Date.now(),
    });
    pushActivity(`Keyword updated: ${keyword.term}.`, 'info');
    await persistState();
    sendJson(res, 200, { ok: true, keyword });
    return;
  }

  if (keywordRoute && req.method === 'DELETE') {
    const [, keywordId] = keywordRoute;
    const keyword = state.keywords.find((item) => item.id === keywordId);
    if (!keyword) {
      sendJson(res, 404, { error: 'not_found', message: 'Keyword was not found.' });
    ...
    pushActivity(`Keyword removed: ${keyword.term}.`, 'warning');
    await persistState();
    sendJson(res, 200, { ok: true, keywordId: keyword.id });
    return;
  }
...
function buildTaskRunSessionId(task, agent) {
  return ['dashboard', 'task', agent?.id, task?.id]
    .filter(Boolean)
    .join('-')
...
async function launchTaskRun(task, agent) {
  ...
  run.requestedSessionId = buildTaskRunSessionId(task, agent);
  ...
  child.on('error', async (error) => {
    ...
    runLogger.appendEvent({
      ...
      details: { failureDetails, requestedSessionId: run.requestedSessionId },
  ...
  child.on('close', async (code) => {
    ...
    const incompleteReason = hasPayload ? detectIncompleteRunReason(output, summaryText, runNote) : null;
    const completedSuccessfully = hasPayload && !incompleteReason;
    const failureText = !hasPayload
      ? run.stderr.trim() || run.stdout.trim() || `Exit code ${code}`
      : incompleteReason || runNote || summaryText || sanitizeRunNote(output) || `Exit code ${code}`;
    ...
    runLogger.appendEvent({
      ...
      message: completedSuccessfully
        ? 'Run completed with payload output.'
        : (hasPayload ? 'Run returned output, but it reported an incomplete result.' : 'Run closed without usable payload output.'),
      details: {
        exitCode: code,
        hasPayload,
        incompleteReason,
        sessionId: agentMeta?.sessionId || run.requestedSessionId || null,
    ...
    if (completedSuccessfully) {
      ...
      runLogger.addArtifact({
        ...
        contentJson: { exitCode: code, stderr: run.stderr, stdout: run.stdout, incompleteReason },
        createdAt: finishedAt,
      });
...
function summarizeTaskRunForPrompt(task) {
  if (!task?.lastRun) {
    return 'No previous run.';
  }

  const summary = limitPromptSection(
    extractSummarySection(task.lastRun.output || '')
      || sanitizeRunNote(task.lastRun.error || '')
      || sanitizeRunNote(task.lastRun.output || ''),
    1200,
  );

  return [
    `status=${task.lastRun.status || 'unknown'}`,
    summary ? `last=${summary}` : null,
  ].filter(Boolean).join(' | ');
}

function buildTaskFamilyContextSection(task) {
  const parentTask = task?.parentTaskId ? findTaskById(task.parentTaskId) : null;
  const childTasks = normalizeTaskDependencyIds(task?.splitChildren)
    .map((taskId) => findTaskById(taskId))
    .filter(Boolean);
  const siblingTasks = parentTask
    ? normalizeTaskDependencyIds(parentTask.splitChildren)
      .map((taskId) => findTaskById(taskId))
      .filter((candidate) => candidate && candidate.id !== task.id)
    : [];

  if (!parentTask && !childTasks.length && !siblingTasks.length) {
    return '';
  }

  return [
    'Split task family context:',
    parentTask
      ? `Parent task: ${parentTask.title} (${parentTask.id})`
      : `Current task is the split parent task (${task.id}).`,
    parentTask?.notes ? `Parent notes: ${limitPromptSection(parentTask.notes, 2000)}` : null,
    siblingTasks.length ? 'Sibling tasks:' : null,
    ...siblingTasks.map((sibling) => `- ${sibling.title} (${sibling.id}) lane=${sibling.lane} ${summarizeTaskRunForPrompt(sibling)}`),
    childTasks.length ? 'Child tasks:' : null,
    ...childTasks.map((child) => `- ${child.title} (${child.id}) lane=${child.lane} ${summarizeTaskRunForPrompt(child)}`),
    'Use this family context to stay aligned with related split work, but do not assume a contract is complete unless a related task actually implemented it.',
  ].filter(Boolean).join('\n');
}
...
function buildTaskPrompt(task, agent, executionContext = null) {
  ...
  const documentationSection = documentationRepos.length
    ...
    : null;
  const taskFamilySection = buildTaskFamilyContextSection(task);
...
async function workspaceHasUncommittedChanges(repoDir) {
  const result = await runCommand(GIT_BIN, ['status', '--porcelain'], { cwd: repoDir }).catch(() => ({ stdout: 'dirty' }));
  return Boolean(result.stdout.trim());
}

async function branchHasUniqueCommits(repoDir, branchName, baseRef) {
  if (!branchName || !baseRef) {
    return true;
  }

  const result = await runCommand(GIT_BIN, ['rev-list', '--count', `${baseRef}..${branchName}`], { cwd: repoDir }).catch(() => ({ stdout: '1' }));
  const count = Number.parseInt(result.stdout.trim(), 10);
  return Number.isFinite(count) ? count > 0 : true;
}

async function refreshTaskWorkspaceBase(cloneDir, workspaceDir, branchName, baseRef) {
  if (!baseRef) {
    return { refreshed: false, reason: 'missing-base-ref' };
  }

  const hasUncommittedChanges = await workspaceHasUncommittedChanges(workspaceDir);
  if (hasUncommittedChanges) {
    return { refreshed: false, reason: 'workspace-dirty' };
  }

  const hasUniqueCommits = await branchHasUniqueCommits(cloneDir, branchName, baseRef);
  if (hasUniqueCommits) {
    return { refreshed: false, reason: 'branch-has-unique-commits' };
  }

  await runCommand(GIT_BIN, ['reset', '--hard', baseRef], { cwd: workspaceDir });
  await runCommand(GIT_BIN, ['clean', '-fd'], { cwd: workspaceDir }).catch(() => {});
  return { refreshed: true, reason: 'reset-to-latest-base' };
}
...
async function ensureTaskRepoWorkspace(task, project, repo, options = {}) {
  ...
  await refreshTaskWorkspaceBase(cloneDir, workspaceDir, branchName, baseRef);
...
async function buildDashboardPayload() {
  ...
  return {
    ...
    keywords: [...state.keywords]
      .sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0) || a.term.localeCompare(b.term))
      .map((keyword) => ({
        ...keyword,
        project: state.projects.find((project) => project.id === keyword.projectId) || null,
      })),
    lanes,
    tasks: [...state.tasks]
    ...
    metrics: {
      ...
      keywordOpportunityCount: state.keywords.filter((keyword) => ['idea', 'researching', 'targeting'].includes(keyword.status)).length,
    },
    openclaw: {
      sessions,
      backgroundTasks,
  ...
  try {
    ...
    return {
      ...
      keywords: Array.isArray(parsed.keywords)
        ? parsed.keywords
            .filter((keyword) => keyword && typeof keyword === 'object')
            .map((keyword) => normalizeKeywordRecord(keyword))
        : [],
      tasks,
...
function sanitizeKeywordValue(value) {
  return String(value || '').trim();
}

function sanitizeKeywordStatus(value) {
  const status = sanitizeKeywordValue(value).toLowerCase();
  return ['idea', 'researching', 'targeting', 'drafting', 'published', 'paused'].includes(status)
    ? status
    : 'idea';
}

function sanitizeKeywordIntent(value) {
  const intent = sanitizeKeywordValue(value).toLowerCase();
  return ['informational', 'commercial', 'transactional', 'navigational', 'local'].includes(intent)
    ? intent
    : 'informational';
}

function sanitizeKeywordPriority(value) {
  const priority = sanitizeKeywordValue(value).toLowerCase();
  return ['high', 'medium', 'low'].includes(priority) ? priority : 'medium';
}

function normalizeKeywordRecord(keyword) {
  return {
    id: String(keyword.id || nextId('keyword')),
    projectId: sanitizeKeywordValue(keyword.projectId),
    market: sanitizeKeywordValue(keyword.market || 'global').toLowerCase(),
    language: sanitizeKeywordValue(keyword.language || 'en').toLowerCase(),
    cluster: sanitizeKeywordValue(keyword.cluster),
    term: sanitizeKeywordValue(keyword.term),
    intent: sanitizeKeywordIntent(keyword.intent),
    status: sanitizeKeywordStatus(keyword.status),
    priority: sanitizeKeywordPriority(keyword.priority),
    opportunityScore: clamp(Math.round(Number(keyword.opportunityScore || 0) || 0), 0, 100),
    source: sanitizeKeywordValue(keyword.source),
    notes: sanitizeKeywordValue(keyword.notes),
    targetPage: sanitizeKeywordValue(keyword.targetPage),
    createdAt: Number(keyword.createdAt || Date.now()),
    updatedAt: Number(keyword.updatedAt || Date.now()),
  };
}

function createKeywordRecord(input, existing = null) {
  const projectId = sanitizeKeywordValue(input?.projectId || existing?.projectId);
  const project = state.projects.find((item) => item.id === projectId) || null;
  if (!project) {
    throw new Error('Keyword must be linked to an existing project.');
  }

  const term = sanitizeKeywordValue(input?.term || existing?.term);
  if (!term) {
    throw new Error('Keyword term is required.');
  }

  return normalizeKeywordRecord({
    id: existing?.id || nextId('keyword'),
    projectId,
    market: input?.market ?? existing?.market ?? 'global',
    language: input?.language ?? existing?.language ?? 'en',
    cluster: input?.cluster ?? existing?.cluster ?? '',
    term,
    intent: input?.intent ?? existing?.intent ?? 'informational',
    status: input?.status ?? existing?.status ?? 'idea',
    priority: input?.priority ?? existing?.priority ?? 'medium',
    opportunityScore: input?.opportunityScore ?? existing?.opportunityScore ?? 0,
    source: input?.source ?? existing?.source ?? '',
    notes: input?.notes ?? existing?.notes ?? '',
    targetPage: input?.targetPage ?? existing?.targetPage ?? '',
...
function detectIncompleteRunReason(output, summaryText = '', runNote = '') {
  const haystack = [summaryText, output, runNote]
    .filter(Boolean)
    .join('\n')
    .trim();
  ...
  const rules = [
    { pattern: /\bapply patch failed\b/i, reason: 'Apply Patch failed.' },
    { pattern: /\bedit: .* failed\b/i, reason: 'An edit step failed.' },
    { pattern: /request timed out before a response was generated/i, reason: 'The run timed out before it produced a usable result.' },
    { pattern: /\bnot completed\b/i, reason: 'The run explicitly reported that the task was not completed.' },
    { pattern: /\b(?:i )?couldn['’]t complete\b/i, reason: 'The run explicitly reported that it could not complete the task.' },
    { pattern: /\b(?:can(?:not|’)t|cannot) honestly call .* complete\b/i, reason: 'The run reported that it could not honestly mark the task complete.' },
    { pattern: /\b(?:did not make code changes|no code changes were made)\b/i, reason: 'The run reported that it made no code changes.' },
    { pattern: /\bblocked from implementing\b/i, reason: 'The run reported that implementation was blocked.' },
    { pattern: /\bstill blocked\b/i, reason: 'The run reported that the task is still blocked.' },
    { pattern: /\bunable to complete\b/i, reason: 'The run reported that it was unable to complete the task.' },
  ];
